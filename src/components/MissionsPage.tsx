...
import { getActiveMissions, getRankProgress } from "@/engine/missions";
...
function SingleMissionCard({ data, onClaim }: { data: MissionProgress; onClaim: (mission: MissionDef) => void }) {
  ...
  return (
    <div className="btn-3d--zinc flex w-[200px] flex-col rounded-lg bg-zinc-600 px-3 py-2">
...
export function MissionsPage() {
  ...
  const { missions, rankInfo } = useGameSelector((state) => {
    const active = getActiveMissions(state.claimedMissions, state.rank);
    const progress = getRankProgress(state.rank, state.claimedMissions.length);
    return {
      missions: active.map((m) => getMissionProgress(m, state)),
      rankInfo: progress,
    };
  }, (a, b) =>
    a.rankInfo.rank === b.rankInfo.rank &&
    a.rankInfo.xp === b.rankInfo.xp &&
    a.missions.length === b.missions.length &&
    a.missions.every((m, i) => m.mission.id === b.missions[i].mission.id && m.progress === b.missions[i].progress && m.isComplete === b.missions[i].isComplete)
  );
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="shrink-0 px-4 pt-3 pb-2">
        <div className="flex items-center gap-2">
          <span className="btn-3d--amber inline-flex h-6 w-6 items-center justify-center rounded bg-amber-500 text-xs font-bold text-white">
            {rankInfo.rank}
          </span>
          <h2 className="text-sm font-bold text-zinc-200">{t.missions.pageTitle}</h2>
          <span className="ml-auto text-xs tabular-nums text-zinc-500">
            {rankInfo.xp}/{rankInfo.xpNeeded} XP
          </span>
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4">
        {missions.length > 0 ? (
          ...
          <div className="flex h-32 items-center justify-center">
            <p className="text-sm text-zinc-500">{t.missions.allCompleted}</p>
          </div>
        )}
      </div>
