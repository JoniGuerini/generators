...
export function OfflineWelcomeCard({ gains, onClose }: OfflineWelcomeCardProps) {
  ...
  const totalResources = lineResourceEntries.reduce(
    (sum, e) => sum.add(e.amount), Decimal.dZero
  );
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-xl border border-zinc-600/80 bg-[#0D0D0D] shadow-2xl">
        <div className="px-5 py-4">
          <h2 id="offline-welcome-title" className="text-lg font-bold text-white">
            {t.offline.title}
          </h2>
          <p className="mt-1 text-sm text-zinc-400">
            {t.offline.awayFor}{" "}
            <span className="font-semibold text-white">{formatTime(offlineSeconds)}</span>
          </p>
        </div>
        ...
        <div className="px-5 pb-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {lineResourceEntries.map(({ line, amount }) => {
              ...
              return (
                <div key={line} className="flex items-center gap-1.5 rounded-lg border border-zinc-600/80 bg-zinc-700/80 px-2.5 py-1.5">
                  <span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded ${classes.bg} text-[9px] font-bold text-white`}>
                    {line}
                  </span>
          ...
          {lineResourceEntries.length > 1 && (
            <div className="mt-2 flex items-center gap-1.5 text-xs text-zinc-500">
              <span className="text-cyan-400">●</span>
              <span>{t.offline.resource}:</span>
              <span className="font-bold tabular-nums text-zinc-300">+{formatNumber(totalResources)}</span>
            </div>
          )}
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            ...
            {hasPending && (
              <div className="flex items-center gap-1.5 rounded-lg border border-zinc-600/80 bg-zinc-700/80 px-2.5 py-1.5">
                ...
                <span className="text-sm font-bold tabular-nums text-purple-200">
